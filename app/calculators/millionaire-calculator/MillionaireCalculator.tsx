...
} from "@/lib/calculators/millionaire";
...
function compute(f: FormState): MillionaireResult | null {
  const age = num(f.currentAge);
  return computeMillionaire({
...
export default function MillionaireCalculator() {
  ...
  const breakdown = result
    ? [
        ...
        { label: "Contributions added", value: result.totalContributed, color: "bg-orange-300" },
    ...
    <div className="space-y-6">
      <form onSubmit={onSubmit} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <p className="mt-0.5 text-sm text-zinc-500">Set your savings and goal, then press Calculate.</p>
          ...
          <div className="mt-5 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label htmlFor="contrib">Monthly saving</Label>
                <div className="relative">
                  <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-zinc-400">$</span>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <div className="relative">
                  <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-zinc-400">$</span>
                  <Input id="target" type="number" min={1} step="any" inputMode="decimal" className="h-11 pl-7" value={form.target} onChange={(e) => set("target", e.target.value)} />
                </div>
              </div>
              <div>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          <p className="text-xs font-bold uppercase tracking-wide text-orange-500">Time to your goal</p>
          {result && result.reached ? (
            <>
              <p className="mt-1 text-4xl font-extrabold tracking-tight text-zinc-900 tabular-nums">
                {formatYears(result.yearsToTarget)}
              </p>
              {result.ageAtTarget !== null && (
                <p className="mt-1 text-xs font-medium text-zinc-500">
                  Reached around age {Math.round(result.ageAtTarget)}
                </p>
              )}
            </>
          ) : (
            <p className="mt-1 text-2xl font-extrabold tracking-tight text-zinc-900">
              Not within {MAX_YEARS} years
            </p>
          )}
          <div className="mt-5 space-y-2">
            {result && result.reached ? (
              ...
              <p className="rounded-lg bg-white/70 px-3 py-2.5 text-sm text-zinc-400">
                Raise your monthly saving or return so the goal is reachable.
              </p>
            )}
          </div>
        </div>
...
function GoalChart({ result, target }: { result: MillionaireResult; target: number }) {
  const W = 640;
  const H = 260;
  ...
  const maxVal = Math.max(target, ...data.map((p) => p.balance)) || 1;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-bold text-zinc-900">Climb to your goal</h3>
        <div className="flex items-center gap-4 text-xs text-zinc-500">
          <span className="flex items-center gap-1.5"><span className="h-2 w-3 rounded-sm bg-orange-400/40" /> Balance</span>
          <span className="flex items-center gap-1.5"><span className="h-0.5 w-3 bg-zinc-400" /> Saved</span>
        </div>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Millionaire savings growth chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <path d={areaPath} fill="url(#mlFill)" />
        <line x1={pad.l} y1={targetY} x2={W - pad.r} y2={targetY} stroke="#16a34a" strokeWidth={1.5} strokeDasharray="5 4" />
        <text x={W - pad.r} y={targetY - 5} textAnchor="end" className="fill-green-600" fontSize={10} fontWeight={700}>Goal {formatCompact(target)}</text>
