...
function compute(f: FormState): CompoundResult | null {
...
export default function CompoundInterestCalculator() {
  ...
  const breakdown = result
    ? [
        { label: "Starting principal", value: principalNum, color: "bg-zinc-300" },
...
function GrowthChart({ result }: { result: CompoundResult }) {
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Investment balance growth chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <path d={areaPath} fill="url(#ciFill)" />
