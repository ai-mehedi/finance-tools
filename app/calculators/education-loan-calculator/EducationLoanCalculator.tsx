...
import {
  computeEducationLoan,
  formatUSD,
  formatUSD2,
  formatCompact,
  type EducationLoanResult,
} from "@/lib/calculators/education-loan";
...
function compute(f: FormState): EducationLoanResult | null {
  return computeEducationLoan({
...
export default function EducationLoanCalculator() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          ...
          {result && (
            <p className="mt-4 text-xs leading-relaxed text-zinc-500">
              You repay over {result.termYears} years, with interest making up{" "}
              <span className="font-semibold text-zinc-600">
                {result.totalPaid > 0 ? ((result.totalInterest / result.totalPaid) * 100).toFixed(0) : 0}%
              </span>{" "}
              of the total.
            </p>
          )}
      ...
      {/* What-if: how different interest rates change the EMI and total interest. */}
      {result && <RateScenarios form={form} />}
    </div>
  );
}

/** Sweeps the interest rate so the user sees how the monthly EMI and total
 *  interest change across a range of rates, plus their own rate. */
function RateScenarios({ form }: { form: FormState }) {
  const base = num(form.annualRatePct) || 0;

  const { rows, highlightIndex } = useMemo(() => {
    const rates = Array.from(new Set([3, 5, 7, 9, 11, base]))
      .filter((r) => r >= 0)
      .sort((a, b) => a - b);
  ...
  const columns: GridColumn[] = [
    ...
    { key: "emi", label: "Monthly EMI", align: "right", format: (v) => formatUSD2(Number(v)) },
    { key: "interest", label: "Total interest", align: "right", format: (v) => formatUSD(Number(v)) },
    { key: "total", label: "Total repayable", align: "right", format: (v) => formatUSD(Number(v)) },
  ];

  return (
    <ScenarioGrid
      title="What if the interest rate were different?"
      caption="Same loan amount and term — only the annual interest rate changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="education-loan-rate-scenarios"
    />
  );
}

function BalanceChart({ result }: { result: EducationLoanResult }) {
  const W = 640;
  const H = 240;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Education loan balance payoff chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <path d={areaPath} fill="url(#eduFill)" />
