...
function compute(f: FormState): LoanEmiResult | null {
  return computeLoanEmi({
...
/** Sweeps the annual interest rate so the user sees how the monthly EMI and
 *  total interest move across a band of rates plus their own. */
function RateScenarios({ form }: { form: FormState }) {
  ...
  const columns: GridColumn[] = [
    ...
    { key: "total", label: "Total payment", align: "right", format: (v) => formatUSD(Number(v)) },
  ];

  return (
    <ScenarioGrid
      title="What if your interest rate were different?"
      caption="Same loan amount and tenure — only the annual rate changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="loan-emi-rate-scenarios"
    />
  );
}

function BalanceChart({ result }: { result: LoanEmiResult }) {
  const W = 640;
  const H = 240;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Loan balance payoff chart">
        ...
        <defs>
          <linearGradient id="loanEmiFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#fb923c" stopOpacity="0.4" />
            <stop offset="100%" stopColor="#fb923c" stopOpacity="0.04" />
          </linearGradient>
        </defs>
        <path d={areaPath} fill="url(#loanEmiFill)" />
