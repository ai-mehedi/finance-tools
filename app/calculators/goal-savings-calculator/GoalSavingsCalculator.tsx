...
import {
  ...
  formatUSD,
  formatCompact,
  type GoalSavingsResult,
} from "@/lib/calculators/goal-savings";
...
function compute(f: FormState): GoalSavingsResult | null {
  return computeGoalSavings({
...
export default function GoalSavingsCalculator() {
  ...
  const [result, setResult] = useState<GoalSavingsResult | null>(() => compute(DEFAULTS));
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (!r) {
      setError("Enter a goal greater than 0, a horizon greater than 0 and non-negative savings.");
      setResult(null);
      return;
    }
    setError(null);
  ...
  const currentNum = num(form.currentSavings) || 0;
  const breakdown = result
    ? [
        { label: "Starting savings", value: currentNum, color: "bg-zinc-300" },
        { label: "Your deposits", value: result.totalContributions, color: "bg-orange-300" },
    ...
    <div className="space-y-6">
      <form onSubmit={onSubmit} className="grid gap-6 lg:grid-cols-5">
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input id="years" type="number" step="any" min={0} inputMode="decimal" className="h-11" value={form.years} onChange={(e) => set("years", e.target.value)} />
              </div>
            </div>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          <p className="text-xs font-bold uppercase tracking-wide text-orange-500">Save per month</p>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            {result
              ? result.monthlyContribution <= 0
                ? "Your current savings already reach the goal."
                : `to reach ${formatUSD(result.goalAmount)}`
              : ""}
          </p>
      ...
      {/* Progress chart */}
      {result && result.schedule.length > 1 && <ProgressChart result={result} />}
    </div>
  );
}

function ProgressChart({ result }: { result: GoalSavingsResult }) {
  const W = 640;
  const H = 260;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-bold text-zinc-900">Progress to goal</h3>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Goal savings progress chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <line x1={pad.l} y1={goalY} x2={W - pad.r} y2={goalY} stroke="#ea580c" strokeWidth={1.5} strokeDasharray="2 3" />
        <text x={W - pad.r} y={goalY - 5} textAnchor="end" className="fill-orange-600" fontSize={10}>Goal</text>
