...
import {
  ...
  formatUSD0,
  formatCompact,
  type AnnuityPayoutResult,
} from "@/lib/calculators/annuity-payout";
...
function compute(f: FormState): AnnuityPayoutResult | null {
  return computeAnnuityPayout({
    ...
    <div className="space-y-6">
      <form onSubmit={onSubmit} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label htmlFor="years">Payout period (years)</Label>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          <p className="text-xs font-bold uppercase tracking-wide text-orange-500">Monthly payout</p>
          <p className="mt-1 text-4xl font-extrabold tracking-tight text-zinc-900 tabular-nums">
            {result ? formatUSD(result.monthlyPayout) : "—"}
          </p>
          <div className="mt-5 space-y-2">
            {result ? (
              <>
                <div className="flex items-center justify-between rounded-lg bg-white/70 px-3 py-2.5">
                  <span className="text-sm font-medium text-zinc-500">Annual payout</span>
                  <span className="text-sm font-bold tabular-nums text-zinc-900">{formatUSD(result.annualPayout)}</span>
                </div>
                <div className="flex items-center justify-between rounded-lg bg-white/70 px-3 py-2.5">
                  <span className="text-sm font-medium text-zinc-500">Total paid out</span>
                  <span className="text-sm font-bold tabular-nums text-zinc-900">{formatUSD0(result.totalPaidOut)}</span>
                </div>
                <div className="flex items-center justify-between rounded-lg bg-white/70 px-3 py-2.5">
                  <span className="text-sm font-medium text-zinc-500">Interest earned</span>
                  <span className="text-sm font-bold tabular-nums text-zinc-900">{formatUSD0(result.totalInterest)}</span>
                </div>
              </>
            ) : (
...
function BalanceChart({ result }: { result: AnnuityPayoutResult }) {
  const W = 640;
  const H = 240;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Annuity balance drawdown chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <path d={areaPath} fill="url(#payoutFill)" />
