...
import {
  computeCompoundSavings,
  formatUSD,
  formatCompact,
  type Frequency,
  type CompoundSavingsResult,
} from "@/lib/calculators/compound-savings";
...
function compute(f: FormState): CompoundSavingsResult | null {
  return computeCompoundSavings({
    startingDeposit: num(f.startingDeposit) || 0,
...
export default function CompoundSavingsCalculator() {
  ...
  const [result, setResult] = useState<CompoundSavingsResult | null>(() => compute(DEFAULTS));
  ...
  const startNum = num(form.startingDeposit) || 0;
  const breakdown = result
    ? [
        { label: "Starting deposit", value: startNum, color: "bg-zinc-300" },
    ...
    <div className="space-y-6">
      <form onSubmit={onSubmit} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <div className="mt-5 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="start">Starting deposit</Label>
                <div className="relative">
                  <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-zinc-400">$</span>
                  <Input id="start" type="number" min={0} step="any" inputMode="decimal" className="h-11 pl-7" value={form.startingDeposit} onChange={(e) => set("startingDeposit", e.target.value)} />
                </div>
              </div>
              <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <Label htmlFor="rate">APY (% / yr)</Label>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          <p className="text-xs font-bold uppercase tracking-wide text-orange-500">Future balance</p>
...
function GrowthChart({ result }: { result: CompoundSavingsResult }) {
  const W = 640;
  const H = 260;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Savings balance growth chart">
        {grid.map((g, i) => (
          <g key={i}>
