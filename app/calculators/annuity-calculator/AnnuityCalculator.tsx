...
const TIMINGS: { value: Timing; label: string }[] = [
  ...
  { value: "begin", label: "Start of period" },
];
...
function compute(f: FormState): AnnuityResult | null {
  return computeAnnuity({
    ...
    <div className="space-y-6">
      <form onSubmit={(e) => e.preventDefault()} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          <h2 className="text-base font-extrabold text-zinc-900">Your annuity</h2>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <Label htmlFor="timing">Payments</Label>
                <Select id="timing" className="h-11" value={form.timing} onChange={(e) => set("timing", e.target.value as Timing)}>
                  {TIMINGS.map((t) => (
      ...
      {/* What-if: how different monthly contributions change future value + interest. */}
      {result && <ContributionScenarios form={form} />}
    </div>
  );
}

/** Sweeps the monthly contribution so the user sees the resulting future value
 *  and total interest at a spread of amounts plus their own value. */
function ContributionScenarios({ form }: { form: FormState }) {
  const base = num(form.monthlyContribution) || 0;

  const { rows, highlightIndex } = useMemo(() => {
    const amounts = Array.from(new Set([0, 100, 250, 500, 1000, base]))
      .filter((c) => c >= 0)
      .sort((a, b) => a - b);
  ...
  return (
    <ScenarioGrid
      title="What if you contributed a different amount each month?"
      caption="Same starting balance, rate and term — only the monthly contribution changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="annuity-contribution-scenarios"
    />
  );
}

function GrowthChart({ result }: { result: AnnuityResult }) {
  const W = 640;
  const H = 260;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Annuity balance growth chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <path d={areaPath} fill="url(#annFill)" />
