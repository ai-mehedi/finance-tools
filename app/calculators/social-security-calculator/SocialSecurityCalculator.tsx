...
export default function SocialSecurityCalculator() {
  ...
  const error = result === null ? "Enter a benefit above 0 and a claiming age between 62 and 70." : null;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* What-if: how the claiming age changes the monthly and annual benefit. */}
      {result && <ClaimAgeScenarios form={form} />}
    </div>
  );
}

/** Sweeps the claiming age (62, 65, FRA, 67, 70 plus the user's age) so they can
 *  compare the monthly and annual benefit at each option. */
function ClaimAgeScenarios({ form }: { form: FormState }) {
  const base = num(form.claimAge);
  const fra = num(form.fullRetirementAge);

  const { rows, highlightIndex } = useMemo(() => {
    const ages = Array.from(new Set([62, 65, fra, 67, 70, base]))
      .filter((a) => Number.isFinite(a) && a >= 62 && a <= 70)
      .sort((a, b) => a - b);

    const built = ages.map((age) => {
      const r = compute({ ...form, claimAge: String(age) });
      return {
        age,
        monthly: r?.monthlyBenefit ?? 0,
        annual: r?.annualBenefit ?? 0,
        pct: r ? `${Math.round(r.factor * 100)}%` : "—",
      };
    });
  ...
  const columns: GridColumn[] = [
    { key: "age", label: "Claim at age" },
    { key: "pct", label: "% of full benefit", align: "right" },
    { key: "monthly", label: "Monthly benefit", align: "right", format: (v) => formatUSD(Number(v)) },
    { key: "annual", label: "Annual benefit", align: "right", format: (v) => formatUSD(Number(v)) },
  ];

  return (
    <ScenarioGrid
      title="What if you claimed at a different age?"
      caption="Same full-retirement-age benefit — only your claiming age changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="social-security-claim-age-scenarios"
    />
  );
}
