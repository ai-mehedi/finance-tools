...
function compute(f: FormState): SavingsGoalResult | null {
...
export default function SavingsGoalCalculator() {
  ...
  const [result, setResult] = useState<SavingsGoalResult | null>(() => compute(DEFAULTS));
  ...
  const breakdown = result
    ? [
        { label: "Starting balance", value: currentNum, color: "bg-zinc-300" },
    ...
    <div className="space-y-6">
      <form onSubmit={onSubmit} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <p className="mt-0.5 text-sm text-zinc-500">Set a target, then press Calculate to see the monthly deposit.</p>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          <p className="text-xs font-bold uppercase tracking-wide text-orange-500">Deposit needed each month</p>
          ...
          {result?.alreadyReached && (
            <p className="mt-2 rounded-lg bg-white/70 px-3 py-2 text-xs font-medium text-emerald-600">
              Your current balance alone reaches the goal in this time, even with no new deposits.
            </p>
          )}
      ...
      {/* Progress chart */}
      {result && result.schedule.length > 1 && <GoalChart result={result} goal={num(form.goalAmount)} />}
    </div>
  );
}

function GoalChart({ result, goal }: { result: SavingsGoalResult; goal: number }) {
  const W = 640;
  const H = 260;
  ...
  const goalVal = Number.isFinite(goal) && goal > 0 ? goal : 1;
  const maxVal = Math.max(goalVal, ...data.map((p) => p.balance)) || 1;
    ...
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 text-xs text-zinc-500">
          ...
          <span className="flex items-center gap-1.5"><span className="h-0.5 w-3 bg-emerald-400" /> Goal</span>
        </div>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Savings goal progress chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <line x1={pad.l} y1={goalY} x2={W - pad.r} y2={goalY} stroke="#34d399" strokeWidth={1.5} strokeDasharray="5 3" />
        {xTicks.map((t) => (
