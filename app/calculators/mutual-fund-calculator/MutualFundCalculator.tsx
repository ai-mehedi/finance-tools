...
import {
  ...
  formatCompact,
  type InvestMode,
  type MutualFundResult,
} from "@/lib/calculators/mutual-fund";

const MODES: { value: InvestMode; label: string }[] = [
...
function compute(f: FormState): MutualFundResult | null {
  return computeMutualFund({
    mode: f.mode,
...
export default function MutualFundCalculator() {
  ...
  const error =
    result === null ? "Enter a horizon greater than 0 and non-negative amounts and rates." : null;
  ...
  const isSip = form.mode === "sip";

  const breakdown = result
    ? [
        { label: "Total invested", value: result.totalInvested, color: "bg-orange-300" },
        { label: "Estimated gain", value: result.estimatedGain, color: "bg-orange-500" },
      ]
    : [];
    ...
    <div className="space-y-6">
      <form onSubmit={(e) => e.preventDefault()} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <p className="mt-0.5 text-sm text-zinc-500">Choose how you invest, then press Calculate.</p>

          <div className="mt-5 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="mode">Investment type</Label>
                <Select id="mode" className="h-11" value={form.mode} onChange={(e) => set("mode", e.target.value as InvestMode)}>
                  {MODES.map((m) => (
              ...
              {isSip ? (
                ...
                <div>
                  <Label htmlFor="lump">Lump sum</Label>
                  <div className="relative">
                    <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-zinc-400">$</span>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <Input id="exp" type="number" min={0} step="any" inputMode="decimal" className="h-11" value={form.expenseRatioPct} onChange={(e) => set("expenseRatioPct", e.target.value)} />
              </div>
              <div>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          <p className="text-xs font-bold uppercase tracking-wide text-orange-500">Estimated value</p>
          <p className="mt-1 text-4xl font-extrabold tracking-tight text-zinc-900 tabular-nums">
            {result ? formatUSD(result.futureValue) : "—"}
          </p>
          {result && (
            <p className="mt-1 text-sm font-medium text-zinc-500">
              net return {result.netReturnPct.toFixed(2)}% / yr after fees
            </p>
          )}
      ...
      {/* What-if: how different expected annual returns change the final value. */}
      {result && <ReturnScenarios form={form} />}
    </div>
  );
}

/** Sweeps the expected annual return so the user sees how the final value and
 *  gain shift at a range of return assumptions plus their own value. */
function ReturnScenarios({ form }: { form: FormState }) {
  const base = num(form.expectedReturnPct) || 0;

  const { rows, highlightIndex } = useMemo(() => {
    const rates = Array.from(new Set([4, 6, 8, 10, 12, 15, base]))
      .filter((r) => Number.isFinite(r) && r >= 0)
      .sort((a, b) => a - b);

    const built = rates.map((rate) => {
      const r = compute({ ...form, expectedReturnPct: String(rate) });
      return {
        rate,
        futureValue: r?.futureValue ?? 0,
        gain: r?.estimatedGain ?? 0,
      };
    });
  ...
  const columns: GridColumn[] = [
    { key: "rate", label: "Return / yr", format: (v) => `${Number(v)}%` },
    { key: "futureValue", label: "Future value", align: "right", format: (v) => formatUSD(Number(v)) },
    { key: "gain", label: "Estimated gain", align: "right", format: (v) => formatUSD(Number(v)) },
  ];

  return (
    <ScenarioGrid
      title="What if returns were higher or lower?"
      caption="Same investment and horizon — only the expected annual return changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="mutual-fund-return-scenarios"
    />
  );
}

function GrowthChart({ result }: { result: MutualFundResult }) {
  const W = 640;
  const H = 260;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Mutual fund growth chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <path d={areaPath} fill="url(#mfFill)" />
