...
import {
  ...
  formatUSD,
  formatCompact,
  type DailySavingsResult,
} from "@/lib/calculators/daily-savings";
...
function compute(f: FormState): DailySavingsResult | null {
  return computeDailySavings({
...
export default function DailySavingsCalculator() {
  const [form, setForm] = useState<FormState>(DEFAULTS);
  const [result, setResult] = useState<DailySavingsResult | null>(() => compute(DEFAULTS));
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <div className="mt-5 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="daily">Amount saved per day</Label>
...
function GrowthChart({ result }: { result: DailySavingsResult }) {
  const W = 640;
  const H = 260;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Daily savings balance growth chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <path d={areaPath} fill="url(#dsFill)" />
