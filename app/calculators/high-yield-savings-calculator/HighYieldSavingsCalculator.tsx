...
import {
  computeHighYieldSavings,
  formatUSD,
  formatCompact,
  type DepositTiming,
  type HighYieldSavingsResult,
} from "@/lib/calculators/high-yield-savings";

const TIMINGS: { value: DepositTiming; label: string }[] = [
...
function compute(f: FormState): HighYieldSavingsResult | null {
  const years = num(f.years);
  return computeHighYieldSavings({
    initialDeposit: num(f.initialDeposit) || 0,
    monthlyDeposit: num(f.monthlyDeposit) || 0,
    apyPct: num(f.apyPct) || 0,
    months: Number.isFinite(years) ? years * 12 : NaN,
    timing: f.timing,
  });
}
...
export default function HighYieldSavingsCalculator() {
  ...
  const [result, setResult] = useState<HighYieldSavingsResult | null>(() => compute(DEFAULTS));
  ...
  const initialNum = num(form.initialDeposit) || 0;
  const breakdown = result
    ? [
        { label: "Opening deposit", value: initialNum, color: "bg-zinc-300" },
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <div className="mt-5 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="initial">Opening deposit</Label>
                <div className="relative">
                  <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-zinc-400">$</span>
                  <Input id="initial" type="number" min={0} step="any" inputMode="decimal" className="h-11 pl-7" value={form.initialDeposit} onChange={(e) => set("initialDeposit", e.target.value)} />
                </div>
              </div>
              <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <Label htmlFor="timing">Deposit timing</Label>
                <Select id="timing" className="h-11" value={form.timing} onChange={(e) => set("timing", e.target.value as DepositTiming)}>
                  {TIMINGS.map((t) => (
...
function SavingsChart({ result }: { result: HighYieldSavingsResult }) {
  const W = 640;
  const H = 260;
  ...
  const depPts = data.map((p) => `${x(p.month).toFixed(1)},${y(p.deposited).toFixed(1)}`);
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="High yield savings balance chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <defs>
          <linearGradient id="hysFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#fb923c" stopOpacity="0.45" />
            <stop offset="100%" stopColor="#fb923c" stopOpacity="0.04" />
          </linearGradient>
        </defs>
        <path d={areaPath} fill="url(#hysFill)" />
        ...
        {xTicks.map((t) => (
          <text key={t} x={x(t)} y={H - 8} textAnchor="middle" className="fill-zinc-400" fontSize={10}>
            {years >= 1 ? `${(t / 12).toFixed(t % 12 === 0 ? 0 : 1)} yr` : `${t} mo`}
          </text>
        ))}
      </svg>
    </div>
