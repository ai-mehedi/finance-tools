...
} from "@/lib/calculators/future-value";
...
function compute(f: FormState): FutureValueResult | null {
...
export default function FutureValueCalculator() {
  ...
  const presentNum = num(form.presentValue) || 0;
  const breakdown = result
    ? [
        { label: "Present value", value: presentNum, color: "bg-zinc-300" },
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <div className="mt-5 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="present">Present value</Label>
                <div className="relative">
                  <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-zinc-400">$</span>
                  <Input id="present" type="number" min={0} step="any" inputMode="decimal" className="h-11 pl-7" value={form.presentValue} onChange={(e) => set("presentValue", e.target.value)} />
                </div>
              </div>
              <div>
      ...
      {/* What-if: how different annual return rates change the future value. */}
      {result && <RateScenarios form={form} />}
    </div>
  );
}

/** Sweeps the annual return rate so the user sees how the future value and
 *  total interest change at 4% / 6% / 8% / 10% / 12% plus their own rate. */
function RateScenarios({ form }: { form: FormState }) {
  const base = num(form.annualRatePct) || 0;

  const { rows, highlightIndex } = useMemo(() => {
    const rates = Array.from(new Set([4, 6, 8, 10, 12, base]))
  ...
  const columns: GridColumn[] = [
    { key: "rate", label: "Annual return", format: (v) => `${Number(v)}%` },
  ...
  return (
    <ScenarioGrid
      title="What if your return rate were different?"
      caption="Same contributions and timeline — only the annual return rate changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="future-value-rate-scenarios"
    />
  );
}

function GrowthChart({ result }: { result: FutureValueResult }) {
  const W = 640;
  const H = 260;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Future value growth chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <path d={areaPath} fill="url(#fvFill)" />
