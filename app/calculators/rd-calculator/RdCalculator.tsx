...
function compute(f: FormState): RdResult | null {
  return computeRd({
    monthlyDeposit: num(f.monthlyDeposit),
...
export default function RdCalculator() {
  ...
  const breakdown = result
    ? [
        ...
        { label: "Instalments", value: result.months, color: "bg-zinc-300", isCount: true },
      ]
    : [];
              ...
              breakdown.map((b) => (
                <div key={b.label} className="flex items-center justify-between rounded-lg bg-white/70 px-3 py-2.5">
                  ...
                  <span className="text-sm font-bold tabular-nums text-zinc-900">
                    {b.isCount ? `${b.value}` : formatUSD(b.value as number)}
                  </span>
                </div>
              ))
            ) : (
...
function RdChart({ result }: { result: RdResult }) {
  const W = 640;
  const H = 260;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Recurring deposit growth chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <path d={areaPath} fill="url(#rdFill)" />
