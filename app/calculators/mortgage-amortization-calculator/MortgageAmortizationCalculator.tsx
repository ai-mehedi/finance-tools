...
} from "@/lib/calculators/mortgage-amortization";
...
export default function MortgageAmortizationCalculator() {
  ...
  const error =
    result === null
      ? "Enter a loan amount and term greater than 0, with a non-negative rate."
      : null;
  ...
  const hasExtra = result ? num(form.extraMonthly) > 0 : false;
    ...
    <div className="space-y-6">
      <form onSubmit={(e) => e.preventDefault()} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <p className="mt-0.5 text-sm text-zinc-500">Enter your mortgage, then press Calculate.</p>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div>
              <Label htmlFor="extra">Extra principal / month (optional)</Label>
              <div className="relative">
                <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-zinc-400">$</span>
          ...
          <div className="mt-5 space-y-2">
            {result ? (
              <>
                ...
                <Row label="Total paid" value={formatUSD(result.totalPayment)} dot="bg-orange-300" />
                <Row label="Payoff time" value={payoffLabel(result.payoffMonths)} dot="bg-zinc-300" />
                {hasExtra && result.interestSaved > 0 && (
                  <div className="mt-2 rounded-lg bg-emerald-50 px-3 py-2.5 text-sm font-semibold text-emerald-700">
                    Extra payments save {formatUSD(result.interestSaved)} and {payoffLabel(result.monthsSaved)}.
                  </div>
                )}
              </>
            ) : (
      ...
      {/* Amortization chart */}
      {result && result.schedule.length > 1 && <AmortChart result={result} />}
...
/** Sweeps the extra monthly principal so the user sees total interest and the
 *  new payoff time at $0 / $100 / $250 / $500 / $1000 plus their own value. */
function ExtraPaymentScenarios({ form }: { form: FormState }) {
  ...
  const { rows, highlightIndex } = useMemo(() => {
    ...
    const built = extras.map((extra) => {
      ...
      return {
        ...
        payoff: r ? payoffLabel(r.payoffMonths) : "—",
      };
    });
  ...
  return (
    <ScenarioGrid
      ...
      csvName="mortgage-amortization-extra-payment-scenarios"
    />
  );
}
...
function AmortChart({ result }: { result: AmortizationResult }) {
  const W = 640;
  const H = 260;
  ...
  const intPts = data.map((p) => `${x(p.year).toFixed(1)},${y(p.cumulativeInterest).toFixed(1)}`);
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Mortgage amortization chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <defs>
          <linearGradient id="amFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#fb923c" stopOpacity="0.45" />
            <stop offset="100%" stopColor="#fb923c" stopOpacity="0.04" />
          </linearGradient>
        </defs>
        <path d={areaPath} fill="url(#amFill)" />
