...
import {
  ...
  formatUSD,
  formatCompact,
  type Five29PlanResult,
} from "@/lib/calculators/529-plan";
...
function compute(f: FormState): Five29PlanResult | null {
  return computeFive29Plan({
...
export default function Five29PlanCalculator() {
  const [form, setForm] = useState<FormState>(DEFAULTS);
  const [result, setResult] = useState<Five29PlanResult | null>(() => compute(DEFAULTS));
  ...
  const breakdown = result
    ? [
        ...
        { label: "Investment earnings", value: result.totalEarnings, color: "bg-orange-500" },
      ]
    : [];
    ...
    <div className="space-y-6">
      <form onSubmit={onSubmit} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          <h2 className="text-base font-extrabold text-zinc-900">Savings plan</h2>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label htmlFor="years">Years until college</Label>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          <p className="text-xs font-bold uppercase tracking-wide text-orange-500">Projected balance</p>
...
function GrowthChart({ result }: { result: Five29PlanResult }) {
  const W = 640;
  const H = 260;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="529 plan balance growth chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <defs>
          <linearGradient id="plan529Fill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#fb923c" stopOpacity="0.45" />
            <stop offset="100%" stopColor="#fb923c" stopOpacity="0.04" />
          </linearGradient>
        </defs>
        <path d={areaPath} fill="url(#plan529Fill)" />
