...
export default function BiweeklyMortgageCalculator() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* What-if: how the interest rate changes the savings from biweekly payments. */}
      {result && <RateScenarios form={form} />}
    </div>
  );
}

/** Sweeps the interest rate so the user sees how the biweekly plan's interest
 *  saved and time saved change at a range of rates plus their own value. */
function RateScenarios({ form }: { form: FormState }) {
  const base = num(form.annualRatePct) || 0;

  const { rows, highlightIndex } = useMemo(() => {
    const rates = Array.from(new Set([3, 4.5, 6, 6.5, 7.5, 9, base]))
      .filter((r) => r >= 0)
      .sort((a, b) => a - b);
    ...
    const built = rates.map((rate) => {
      ...
      return {
        ...
        timeSaved: r ? formatMonths(r.monthsSaved) : "—",
      };
    });
  ...
  const columns: GridColumn[] = [
    ...
    { key: "timeSaved", label: "Time saved", align: "right" },
  ];

  return (
    <ScenarioGrid
      title="What if your rate were different?"
      caption="Same loan and term — only the interest rate changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="biweekly-mortgage-rate-scenarios"
    />
  );
}
