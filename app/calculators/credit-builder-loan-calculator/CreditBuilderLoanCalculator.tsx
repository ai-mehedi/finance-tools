...
import {
  computeCreditBuilderLoan,
  formatUSD,
  formatCompact,
  type CreditBuilderResult,
} from "@/lib/calculators/credit-builder-loan";
...
function compute(f: FormState): CreditBuilderResult | null {
  return computeCreditBuilderLoan({
...
export default function CreditBuilderLoanCalculator() {
  const { state: form, set, reset, shareUrl } = useCalcState<FormState>(DEFAULTS);
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => compute(form), [form]);
  const error = result === null ? "Enter a loan amount and term in months greater than 0." : null;
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="mt-5 space-y-2">
            {result ? (
              <>
                ...
                <div className="flex items-center justify-between rounded-lg bg-white/70 px-3 py-2.5">
                  <span className="text-sm font-medium text-zinc-500">Interest cost</span>
                  <span className="text-sm font-bold tabular-nums text-zinc-900">{formatUSD(result.totalInterest)}</span>
                </div>
                <div className="flex items-center justify-between rounded-lg bg-white/70 px-3 py-2.5">
                  <span className="text-sm font-medium text-zinc-500">Amount returned</span>
                  <span className="text-sm font-bold tabular-nums text-zinc-900">{formatUSD(result.amountReturned)}</span>
                </div>
              </>
            ) : (
      ...
      {result && result.schedule.length > 1 && <SavedChart result={result} />}

      {/* What-if: how a different APR changes interest cost and monthly payment. */}
      {result && <RateScenarios form={form} />}
    </div>
  );
}

/** Sweeps the APR so the user sees how the interest cost and monthly payment
 *  change at a range of rates plus their own value. */
function RateScenarios({ form }: { form: FormState }) {
  const base = num(form.annualRatePct) || 0;

  const { rows, highlightIndex } = useMemo(() => {
    const rates = Array.from(new Set([0, 6, 10, 12, 18, 24, base]))
      .filter((rt) => rt >= 0)
      .sort((a, b) => a - b);
  ...
  const columns: GridColumn[] = [
    { key: "rate", label: "APR", format: (v) => `${Number(v)}%` },
    { key: "monthly", label: "Monthly payment", align: "right", format: (v) => formatUSD(Number(v)) },
    { key: "interest", label: "Interest cost", align: "right", format: (v) => formatUSD(Number(v)) },
  ];

  return (
    <ScenarioGrid
      title="What if the APR were different?"
      caption="Same loan amount and term — only the interest rate changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="credit-builder-loan-rate-scenarios"
    />
  );
}

function SavedChart({ result }: { result: CreditBuilderResult }) {
  const W = 640;
  const H = 240;
  ...
  const maxVal = Math.max(...data.map((p) => p.saved)) || 1;

  const x = (mo: number) => pad.l + (mo / months) * innerW;
  const y = (v: number) => pad.t + innerH - (v / maxVal) * innerH;

  const pts = data.map((p) => `${x(p.month).toFixed(1)},${y(p.saved).toFixed(1)}`);
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <h3 className="mb-3 text-sm font-bold text-zinc-900">Savings built up over time</h3>
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Credit builder loan savings chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <defs>
          <linearGradient id="cblFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#fb923c" stopOpacity="0.4" />
            <stop offset="100%" stopColor="#fb923c" stopOpacity="0.04" />
          </linearGradient>
        </defs>
        <path d={areaPath} fill="url(#cblFill)" />
