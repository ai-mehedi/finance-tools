...
export default function HomeLoanEmiCalculator() {
  ...
  const error = result === null ? "Enter a loan amount and tenure greater than 0." : null;
...
/** Sweeps the annual interest rate so the user sees how the EMI and total
 *  interest move at a range of rates plus their own current rate. */
function RateScenarios({ form }: { form: FormState }) {
  const base = num(form.annualRatePct) || 0;

  const { rows, highlightIndex } = useMemo(() => {
    const rates = Array.from(new Set([4, 5, 6, 7, 8, base]))
      .filter((rate) => rate >= 0)
      .sort((a, b) => a - b);
  ...
  return (
    <ScenarioGrid
      ...
      csvName="home-loan-emi-rate-scenarios"
    />
  );
}
