...
export default function MortgageOverpaymentCalculator() {
  ...
  const error =
    result === null
      ? "Enter a balance and remaining term greater than 0, and a non-negative rate."
      : null;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* What-if: how different monthly overpayments change interest saved + payoff time. */}
      {result && <OverpaymentScenarios form={form} />}
    </div>
  );
}

/** Sweeps the monthly overpayment so the user sees interest saved and the new
 *  payoff time at $0 / $100 / $250 / $500 / $1000 plus their own value. */
function OverpaymentScenarios({ form }: { form: FormState }) {
  const base = num(form.monthlyOverpayment) || 0;

  const { rows, highlightIndex } = useMemo(() => {
    const overpayments = Array.from(new Set([0, 100, 250, 500, 1000, base]))
      .filter((e) => e >= 0)
      .sort((a, b) => a - b);

    const built = overpayments.map((overpay) => {
      const r = compute({ ...form, monthlyOverpayment: String(overpay) });
      return {
        overpay,
        saved: r?.interestSaved ?? 0,
        sooner: r ? monthsToText(r.monthsSaved) : "—",
        interest: r?.overTotalInterest ?? 0,
      };
    });

    return { rows: built, highlightIndex: built.findIndex((r) => r.overpay === base) };
  }, [form, base]);

  const columns: GridColumn[] = [
    { key: "overpay", label: "Overpayment / month", format: (v) => formatUSD(Number(v)) },
    { key: "sooner", label: "Cleared sooner", align: "right" },
    { key: "saved", label: "Interest saved", align: "right", format: (v) => formatUSD(Number(v)) },
    { key: "interest", label: "Interest paid", align: "right", format: (v) => formatUSD(Number(v)) },
  ];

  return (
    <ScenarioGrid
      title="What if you overpaid more each month?"
      caption="Same mortgage — only the monthly overpayment changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="mortgage-overpayment-scenarios"
    />
  );
}
