...
function compute(f: FormState): LumpsumResult | null {
...
export default function LumpsumCalculator() {
  ...
  const error = result === null ? "Enter a positive investment amount and a number of years greater than 0." : null;
  ...
  const breakdown = result
    ? [
        { label: "Invested amount", value: result.investedAmount, color: "bg-zinc-300" },
    ...
    <div className="space-y-6">
      <form onSubmit={(e) => e.preventDefault()} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <p className="mt-0.5 text-sm text-zinc-500">Enter a one-time amount, expected return and horizon, then press Calculate.</p>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          ...
          {result && (
            <p className="mt-1 text-sm font-medium text-zinc-500">
              {result.absoluteReturnPct.toFixed(0)}% absolute return
            </p>
          )}
      ...
      {/* What-if: how different expected returns change the maturity value. */}
      {result && <ReturnScenarios form={form} />}
    </div>
  );
}

/** Sweeps the expected annual return so the user sees the maturity value and
 *  total gains at 8% / 10% / 12% / 15% / 18% plus their own rate. */
function ReturnScenarios({ form }: { form: FormState }) {
  const base = num(form.annualRatePct) || 0;

  const { rows, highlightIndex } = useMemo(() => {
    const rates = Array.from(new Set([8, 10, 12, 15, 18, base]))
    ...
    return { rows: built, highlightIndex: built.findIndex((row) => row.rate === base) };
  }, [form, base]);

  const columns: GridColumn[] = [
    { key: "rate", label: "Expected return", format: (v) => `${Number(v)}%` },
    { key: "maturity", label: "Maturity value", align: "right", format: (v) => formatUSD(Number(v)) },
    { key: "returns", label: "Estimated returns", align: "right", format: (v) => formatUSD(Number(v)) },
  ];

  return (
    <ScenarioGrid
      title="What if your return was different?"
      caption="Same investment and horizon — only the expected annual return changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="lumpsum-return-scenarios"
    />
  );
}

function GrowthChart({ result }: { result: LumpsumResult }) {
  const W = 640;
  const H = 260;
  ...
  const investedY = y(result.investedAmount);
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Lumpsum growth chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <defs>
          <linearGradient id="lumpFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#fb923c" stopOpacity="0.45" />
            <stop offset="100%" stopColor="#fb923c" stopOpacity="0.04" />
          </linearGradient>
        </defs>
        <path d={areaPath} fill="url(#lumpFill)" />
        <line x1={pad.l} y1={investedY} x2={W - pad.r} y2={investedY} stroke="#a1a1aa" strokeWidth={1.5} strokeDasharray="4 3" />
