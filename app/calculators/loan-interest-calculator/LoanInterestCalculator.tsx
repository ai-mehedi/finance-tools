...
export default function LoanInterestCalculator() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* What-if: how different interest rates change total interest + monthly payment. */}
 ...
 *  total interest shift at a range of rates plus their own value. */
function RateScenarios({ form }: { form: FormState }) {
  const base = num(form.annualRatePct) || 0;

  const { rows, highlightIndex } = useMemo(() => {
    const rates = Array.from(new Set([3, 5, 7.5, 10, 12, base]))
      .filter((rate) => rate >= 0)
      .sort((a, b) => a - b);
  ...
  const columns: GridColumn[] = [
    ...
    { key: "total", label: "Total repaid", align: "right", format: (v) => formatUSD(Number(v)) },
  ];
  ...
  return (
    <ScenarioGrid
      ...
      csvName="loan-interest-rate-scenarios"
    />
  );
}
