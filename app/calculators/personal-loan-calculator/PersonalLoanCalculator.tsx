...
function compute(f: FormState): PersonalLoanResult | null {
  return computePersonalLoan({
    amount: num(f.amount),
...
export default function PersonalLoanCalculator() {
  ...
  const breakdown = result
    ? [
        { label: "Principal", value: result.amount, color: "bg-zinc-300" },
        { label: "Total interest", value: result.totalInterest, color: "bg-orange-500" },
      ]
    : [];

  const payoffYears = result ? (result.payoffMonths / 12).toFixed(1) : "";
    ...
    <div className="space-y-6">
      <form onSubmit={(e) => e.preventDefault()} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <p className="mt-0.5 text-sm text-zinc-500">Enter the loan terms, then press Calculate.</p>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          ...
          {result && (
            <p className="mt-1 text-sm text-zinc-500">Paid off in about {payoffYears} years</p>
          )}
...
function BalanceChart({ result }: { result: PersonalLoanResult }) {
  const W = 640;
  const H = 260;
  ...
  const maxVal = Math.max(...data.map((p) => p.balance), result.amount) || 1;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-bold text-zinc-900">Balance over the loan</h3>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Loan balance over time chart">
        {grid.map((g, i) => (
          <g key={i}>
