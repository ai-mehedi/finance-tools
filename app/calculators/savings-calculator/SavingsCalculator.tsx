...
function compute(f: FormState): SavingsResult | null {
...
export default function SavingsCalculator() {
  ...
  const [result, setResult] = useState<SavingsResult | null>(() => compute(DEFAULTS));
  ...
  const breakdown = result
    ? [
        { label: "Starting deposit", value: initialNum, color: "bg-zinc-300" },
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <div className="mt-5 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="initial">Starting deposit</Label>
              ...
              <div>
                ...
                <div className="relative">
                  <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-zinc-400">$</span>
                  <Input id="deposit" type="number" min={0} step="any" inputMode="decimal" className="h-11 pl-7" value={form.monthlyDeposit} onChange={(e) => set("monthlyDeposit", e.target.value)} />
                </div>
              </div>
            </div>
...
function GrowthChart({ result }: { result: SavingsResult }) {
  const W = 640;
  const H = 260;
  ...
  const depoPts = data.map((p) => `${x(p.year).toFixed(1)},${y(p.deposited).toFixed(1)}`);
  const areaPath = `M${x(0)},${y(0)} L${balPts.join(" L")} L${x(years)},${y(0)} Z`;
  const balLine = `M${balPts.join(" L")}`;
  const depoLine = `M${depoPts.join(" L")}`;
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-bold text-zinc-900">Savings over time</h3>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Savings growth chart">
        {grid.map((g, i) => (
          <g key={i}>
        ...
        <path d={areaPath} fill="url(#svFill)" />
        <path d={balLine} fill="none" stroke="#f97316" strokeWidth={2.5} strokeLinejoin="round" strokeLinecap="round" />
        <path d={depoLine} fill="none" stroke="#a1a1aa" strokeWidth={1.5} strokeDasharray="4 3" strokeLinejoin="round" />
        {xTicks.map((t) => (
