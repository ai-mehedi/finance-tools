...
function compute(f: FormState): LoanPayoffResult | null {
  return computeLoanPayoff({
...
export default function LoanPayoffCalculator() {
  ...
  const error = result === null
    ? "Enter a positive balance and a monthly payment large enough to cover the monthly interest."
    : null;
  ...
  const breakdown = result
    ? [
        { label: "Time to debt-free", value: formatDuration(result.monthsToPayoff), color: "bg-orange-500" },
        { label: "Total interest paid", value: formatUSD(result.totalInterest), color: "bg-orange-300" },
        { label: "Interest saved", value: formatUSD(result.interestSaved), color: "bg-emerald-400" },
        { label: "Time saved", value: formatDuration(result.monthsSaved), color: "bg-zinc-300" },
      ]
    : [];
    ...
    <div className="space-y-6">
      <form onSubmit={(e) => e.preventDefault()} className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm lg:col-span-3">
          ...
          <p className="mt-0.5 text-sm text-zinc-500">Enter your balance and payments, then press Calculate.</p>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-orange-50 to-amber-50 p-6 shadow-sm lg:col-span-2">
          ...
          <p className="mt-1 text-4xl font-extrabold tracking-tight text-zinc-900 tabular-nums">
            {result ? formatDuration(result.monthsToPayoff) : "—"}
          </p>
...
/** Sweeps the extra monthly payment so the user sees the new payoff time and
 *  interest saved at $0 / $50 / $100 / $250 / $500 plus their own value. */
function ExtraPaymentScenarios({ form }: { form: FormState }) {
  const base = num(form.extraPayment) || 0;
  ...
  const { rows, highlightIndex } = useMemo(() => {
    ...
    const built = extras
      .map((extra) => {
        const r = compute({ ...form, extraPayment: String(extra) });
        if (!r) return null;
        return {
          extra,
          payoff: formatDuration(r.monthsToPayoff),
          interest: r.totalInterest,
          saved: r.interestSaved,
        };
      })
  ...
  return (
    <ScenarioGrid
      ...
      caption="Same loan — only the extra monthly payment changes."
      columns={columns}
      rows={rows}
      highlightIndex={highlightIndex}
      csvName="loan-payoff-extra-payment-scenarios"
    />
  );
}

function PayoffChart({ result }: { result: LoanPayoffResult }) {
  const W = 640;
  const H = 260;
    ...
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 text-xs text-zinc-500">
          ...
          <span className="flex items-center gap-1.5"><span className="h-0.5 w-3 bg-zinc-400" /> Minimum only</span>
        </div>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Loan payoff balance chart">
        {grid.map((g, i) => (
          <g key={i}>
