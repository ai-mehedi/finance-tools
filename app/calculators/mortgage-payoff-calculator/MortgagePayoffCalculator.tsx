...
export default function MortgagePayoffCalculator() {
  ...
  const [result, setResult] = useState<MortgagePayoffResult | null>(() => compute(DEFAULTS));
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (!r) {
      setError("Enter a balance and term greater than 0, plus a non-negative rate and extra payment.");
      setResult(null);
      return;
    }
    if (!Number.isFinite(r.acceleratedMonths)) {
      setError("The payment does not cover the interest. Lower the rate or check the term.");
      setResult(null);
      return;
    }
    setError(null);
