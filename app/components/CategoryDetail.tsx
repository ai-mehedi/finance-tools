        ...
        <div className="relative mx-auto container px-6 py-12">
          <div className="flex items-center gap-4">
            {category.thumbnail && (
              ...
              <img src={category.thumbnail} alt="" referrerPolicy="no-referrer" className="h-16 w-16 object-contain" />
            )}
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {tools.map((tool) => (
                  <Link key={tool._id} href={`/tools/${tool.slug}`} className="group flex flex-col rounded-2xl border border-zinc-200 bg-white p-5 transition-all hover:-translate-y-0.5 hover:shadow-md">
                    ...
                    <p className="mt-1.5 line-clamp-2 flex-1 text-xs leading-relaxed text-zinc-500">{tool.description}</p>
                    <span className="mt-4 inline-flex w-fit items-center gap-1.5 text-xs font-bold text-orange-600">
