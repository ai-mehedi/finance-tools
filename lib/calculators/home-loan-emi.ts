// Pure logic for the Home Loan EMI Calculator. Computes the monthly EMI, total
// interest and total payment for a home loan, plus a yearly amortization
// schedule for charting the remaining balance over time.
//
// EMI = P * r * (1+r)^n / ((1+r)^n - 1)
//   P = loan principal, r = monthly rate (annualRatePct/100/12), n = months.
// If r == 0, EMI = P / n.

export interface HomeLoanEmiInput {
  loanAmount: number;
  annualRatePct: number;
  termYears: number;
}

export interface HomeLoanEmiYearPoint {
  year: number;
  balance: number;
...
export interface HomeLoanEmiResult {
  ...
  payoffYears: number;
  schedule: HomeLoanEmiYearPoint[];
}

export function computeHomeLoanEmi(input: HomeLoanEmiInput): HomeLoanEmiResult | null {
  ...
  let cumInterest = 0;
  let cumPrincipal = 0;
  const schedule: HomeLoanEmiYearPoint[] = [
