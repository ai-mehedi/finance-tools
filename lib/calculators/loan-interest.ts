// Pure logic for the Loan Interest Calculator.
// Uses the standard amortizing loan (EMI) formula to find the fixed monthly
// payment, then amortizes month by month to total the interest paid and build
// a per-year balance schedule for charting.
...
export interface LoanInterestYearPoint {
  year: number;
  balance: number;
...
export function computeLoanInterest(
  ...
  let cumInterest = 0;
  let cumPrincipal = 0;
  const schedule: LoanInterestYearPoint[] = [
